...
| REQ-QUA-06 | Backend tests, lint and types run in CI on every PR touching the backend. | Planned | **Gap.** These gates exist today only as local hooks, so a PR raised from an environment without them configured is unguarded. A workflow should run the fast tests, `ruff check`, and `mypy` on `backend/**`. | — |
