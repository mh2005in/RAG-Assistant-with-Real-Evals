...
| --- | --- | --- | --- | --- |
| REQ-EXT-01 | Detect a PDF upload and extract its text page by page, with per-page stats (chars, words, raw sentence count, token estimate). | Done | `POST /process` on a PDF returns `processed: true` and `doc_type: "pdf"`, one entry per page; a file whose type cannot be identified is reported as `unknown` and stored as nothing. (Other types are ingested too — see `REQ-EXT-04`.) | [file_processing.py](../backend/services/file_processing.py), [test_file_processing.py](../backend/tests/test_file_processing.py) |
...
| REQ-EXT-04 | Ingest non-PDF document types (DOCX, HTML, plain text) behind the same extraction step. | Done | `POST /process` accepts the type and reports it in `doc_type`; per-page stats degrade gracefully for paginationless formats. | [extraction/](../backend/services/extraction/) — one `Extractor` per type behind one interface; [file_processing.py](../backend/services/file_processing.py) sniffs the type and delegates. [test_extraction.py](../backend/tests/test_extraction.py), [test_file_processing.py](../backend/tests/test_file_processing.py) and [test_api.py](../backend/tests/test_api.py) cover detection, the one-page degradation and the endpoint reporting each type. [extraction_formats.json](../backend/evals/results/extraction_formats.json) is what proves no container loses text: all four score recall/precision/order 1.000 against the same source, with two control arms showing the metrics can fail. |
...
| ID | Requirement | Status | Acceptance | Evidence |
| --- | --- | --- | --- | --- |
| REQ-GEN-01 | `POST /answer` retrieves context, augments the prompt with it, and returns a grounded answer plus its source chunks. | Done | The response carries the answer and the sources it was grounded in; citations refer to those sources, and the grounding is measured rather than asserted. | [answering.py](../backend/services/answering.py), [test_answering.py](../backend/tests/test_answering.py) for the response shape; [answer_faithfulness.json](../backend/evals/results/answer_faithfulness.json) (`REQ-EVL-06`) for the quality claim — answers generated through the shipped prompt score 0.92/1.00 faithfulness across the two eval datasets, against 0.58/0.00 on a distractor context built from the same documents. |
...
| REQ-EVL-02 | Every pipeline stage is measured by a **reproducible, checked-in** eval whose results are regenerable artifacts, not screenshots. | Done | All six stages have one: extraction (two — [extraction_fidelity_eval.py](../backend/evals/extraction_fidelity_eval.py) → [extraction_fidelity.json](../backend/evals/results/extraction_fidelity.json) for ways of reading a PDF, and [extraction_formats_eval.py](../backend/evals/extraction_formats_eval.py) → [extraction_formats.json](../backend/evals/results/extraction_formats.json) for DOCX/HTML/text against that PDF baseline, `REQ-EXT-04`), chunking ([fixed_size_chunking.json](../backend/evals/results/fixed_size_chunking.json), [chunking_strategies.json](../backend/evals/results/chunking_strategies.json)), embedding ([embedding_quality_eval.py](../backend/evals/embedding_quality_eval.py) → [embedding_quality.json](../backend/evals/results/embedding_quality.json)), storage ([storage_index_eval.py](../backend/evals/storage_index_eval.py) → [storage_index.json](../backend/evals/results/storage_index.json)), retrieval ([retrieval_ranking.json](../backend/evals/results/retrieval_ranking.json), `REQ-EVL-04`) and generation ([answer_faithfulness.json](../backend/evals/results/answer_faithfulness.json), `REQ-EVL-06`). Every result is committed JSON, regenerated by one command. The extraction, embedding, storage and generation evals each carry a control arm that shows the metric separates; the chunking and retrieval evals predate that convention and compare real candidates only. | [evals/](../backend/evals/), [results/](../backend/evals/results/), [test_stage_evals.py](../backend/tests/test_stage_evals.py) |
...
| ID | Requirement | Status | Acceptance | Evidence |
| --- | --- | --- | --- | --- |
| REQ-OPS-01 | The whole system runs from one command as frontend + app + Postgres/pgvector + Ollama, each with a healthcheck, plus a one-shot model puller — from **any** checkout, the root one or a worktree. | Done | All four containers report healthy, the puller exits 0, and both `/openapi.json` and the SPA root serve. Running `up` from a worktree adopts the same project rather than starting a colliding second copy: the Compose project name is pinned in the file, so it does not vary with the directory. | [docker-compose.yml](../docker-compose.yml) |
