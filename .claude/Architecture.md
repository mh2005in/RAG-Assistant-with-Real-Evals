...
| D16 | One pinned Compose project name shared by every checkout, rather than per-directory isolation | The services carry fixed `container_name`s while Compose derives the project from the directory, so a stack started in a worktree became a second project competing for the same names and ports — and the post-merge hook then deleted the worktree, stranding containers whose bind mounts pointed nowhere and which blocked every later `up`. Pinning the name makes `up` adopt and `down` clean up from anywhere | Two checkouts cannot run the stack at once — no loss, since they already shared the published ports — and all checkouts share one set of volumes |
