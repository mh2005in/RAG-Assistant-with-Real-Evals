...
- Keep eval datasets, prompts, and results reproducible — check in the eval code and config; treat scores as regenerable artifacts, not screenshots. Every stage has an eval today (see the [`eval-runner`](.claude/agents/eval-runner.md) table); a new extractor, strategy or model joins its stage's eval as another arm in the same change. Extraction has two, because the stage asks two questions: `extraction_fidelity_eval.py` compares ways of reading *one* format (a new PyMuPDF mode or an OCR pass is an arm there), and `extraction_formats_eval.py` compares *formats* against the PDF baseline (a new source type is an arm there).
- Evals run offline against the checked-in fixtures, but most of them embed text and so need a running Ollama — only the two extraction evals ([`extraction_fidelity_eval.py`](backend/evals/extraction_fidelity_eval.py), [`extraction_formats_eval.py`](backend/evals/extraction_formats_eval.py)) and the fixed-size sweep need nothing. [`storage_index_eval.py`](backend/evals/storage_index_eval.py) additionally needs a live Postgres, because it measures the HNSW index and a stand-in for a database cannot be approximate the way the real one is.
...
- **One service class per endpoint — don't over-modularize.** Each processing step is a method on that class, not a module per step (e.g. everything behind `/process` lives on `FileProcessing` in `backend/services/file_processing.py`). Only split into a new service once a second endpoint needs it. The exception is a step that is a *set of interchangeable strategies* an eval has to run against each other — chunking (`Chunker`) and extraction (`Extractor`) — which lives in its own stage package; choosing between them still happens on the service class.
...
- **One Compose project across every checkout.** `docker-compose.yml` pins `name:`, so the root checkout and every worktree resolve to the same project: `up` from a worktree adopts the running containers rather than colliding with them over the fixed `rag-*` container names, and `down` from anywhere stops the stack. Don't remove the pin — without it a deleted worktree leaves orphaned containers that block every later `up`, with bind mounts pointing at a directory that no longer exists. Two checkouts still cannot run the stack at once (they share the published ports).
...
- **The Docker stack is shared, not per-worktree.** `docker compose up` from a worktree adopts the one project (see [Local stack](#local-stack-docker-compose)), so it needs no separate teardown before the worktree is removed — and removing a worktree cannot strand containers the way it used to.
