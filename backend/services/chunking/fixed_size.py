...
Splits the page text into fixed-length windows of ``chunk_size`` *words*. This is
the simplest possible baseline: it ignores document structure entirely, which is
exactly what makes it a useful reference point for structure-aware strategies to
beat in evals.

The chunking unit is whitespace-split words. Words are re-joined with single
spaces, so original intra-page whitespace is normalized. Overlap is not applied —
the request carries no overlap parameter.

Page exclusion is applied upstream (it is common to every strategy), so this
chunker simply chunks whatever pages it is handed; an excluded page arrives empty
and contributes no words.
"""
...
class FixedSizeChunker:
    ...
    The words of every page are taken in order and grouped into
    ``chunk_size``-word chunks with no overlap.
    """
    ...
    def chunk_with_pages(self, pages: list[str]) -> list[tuple[int, str]]:
        ...
        """
        # Flatten the pages into a stream of (word, source page) pairs so a
